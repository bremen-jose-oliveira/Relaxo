...
export default function SettingsScreen() {
  ...
  const showInstallLatestBuild =
    __DEV__ || versionInfo.channel === 'preview' || versionInfo.channel === 'development';
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const result = await importCareEvents(importPreview, {
        babyName: baby?.name?.trim() || undefined,
      });
      ...
      if (result.createdBaby) {
        const usedName = profile.name?.trim() || baby?.name?.trim() || 'Baby';
        const usedBirth =
          profile.birthDate ??
          ...
          <Card style={{ marginBottom: spacing.lg }}>
            {authUser ? (
              <>
                ...
                {householdId && inviteCode && !showJoinOther ? (
                  <Pressable
                    onPress={() => setShowJoinOther(true)}
                    style={{ marginBottom: spacing.sm, paddingVertical: spacing.sm }}>
                    <Text style={{ color: colors.tint, fontWeight: '600', fontSize: 15 }}>
                      {t('profile.switchOrJoinHousehold')}
                    </Text>
                  </Pressable>
                ) : (
                  <>
                    {householdId && inviteCode ? (
                      <Pressable
                        onPress={() => {
                          setShowJoinOther(false);
                          setJoinCode('');
                        }}
                        style={{ marginBottom: spacing.sm, paddingVertical: spacing.xs }}>
                        <Text style={{ color: colors.textSecondary, fontWeight: '600', fontSize: 14 }}>
                          {t('profile.hideJoinHousehold')}
                        </Text>
                      </Pressable>
                    ) : null}
                    <Text
